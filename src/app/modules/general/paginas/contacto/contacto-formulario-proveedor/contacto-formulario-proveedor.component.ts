...
@Component({
  selector: 'app-contacto-formulario-proveedor',
  standalone: true,
  templateUrl: './contacto-formulario-proveedor.component.html',
  styleUrls: ['./contacto-formulario-proveedor.component.scss'],
  animations: [
    trigger('fadeInOut', [
      state(
        'void',
...
export default class ContactoFormularioProveedorComponent
  extends General
  ...
  @Input() set datosContacto(valor: Partial<any> | null) {
    if (valor) {
      // Esperar a que el formulario esté creado
      if (this.formularioContacto) {
        this.actualizarFormularioDesdeObjeto(valor);
      } else {
        // Guardar el valor temporalmente para aplicarlo luego
        this._datosPendientes = valor;
      }
    }
  }
  ...
  private _datosPendientes: Partial<any> | null = null;
  ...
  ngOnInit() {
    ...
    //Aplicar datos recibidos por @Input() si llegaron antes de iniciar el formulario
    if (this._datosPendientes) {
      this.actualizarFormularioDesdeObjeto(this._datosPendientes);
  ...
  private actualizarFormularioDesdeObjeto(datos: Partial<Contacto>): void {

    this.formularioContacto.patchValue(
      {
        numero_identificacion: datos.numero_identificacion ?? '',
        identificacion: datos.identificacion_id ?? '',
        codigo: datos.codigo ?? '',
        ...
        ciudad_nombre: datos.ciudad_nombre ?? '',
        direccion: datos.direccion ?? '',
        telefono: datos.telefono ?? '',
        celular: datos.celular ?? '',
        correo: datos.correo ?? '',
        tipo_persona: datos.tipo_persona_id ?? 1,
        regimen: datos.regimen_id ?? 1,
        barrio: datos.barrio ?? null,
        precio: datos.precio_id ?? '',
        plazo_pago: datos.plazo_pago_id ?? '',
        plazo_pago_proveedor: datos.plazo_pago_proveedor_id,
        asesor: datos.asesor_id ?? '',
        cliente: datos.cliente ?? this.esCliente,
        proveedor: datos.proveedor ?? this.esProvedor,
        empleado: datos.empleado ?? false,
        correo_facturacion_electronica: datos.correo_facturacion_electronica ?? '',
        cuenta_banco_clase: datos.cuenta_banco_clase_id ?? '',
        banco_nombre: datos.banco_nombre ?? '',
        banco: datos.banco_id ?? '',
