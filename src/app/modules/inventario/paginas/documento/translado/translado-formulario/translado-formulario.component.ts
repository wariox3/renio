...
import { FormularioTrasladoService } from '@modulos/inventario/service/formulario-traslado.service';
...
@Component({
  selector: 'app-translado-formulario',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './translado-formulario.component.html',
  styleUrl: './translado-formulario.component.scss',
})
export default class TransladoFormularioComponent
  extends General
  implements OnInit
{
  private _formBuilder = inject(FormBuilder);
  private _entradaService = inject(EntradaService);
  private _formularioEntradaService = inject(FormularioTrasladoService);
  ...
  agregarDetalle() {
    ...
    const pagoFormGroup = this._formBuilder.group({
      ...
      id: [null],
      operacion_inventario: [1, Validators.required],
      tipo_registro: ['I'],
      subtotal: [0],
      total_bruto: [0],
      total: [0],
  ...
  onOperacionInventarioChange(event: any, indexFormulario: number) {
    if(event.target.value !== null){
      this.detalles.controls[indexFormulario].patchValue({
        operacion_inventario: Number(event.target.value),
      });
    }
    this.changeDetectorRef.detectChanges();
  }
  ...
  eliminarItem(indexFormulario: number) {
    ...
    itemsActualizados.forEach((item: any, i: number) => {
      ...
      this.detalles.controls[i].patchValue({
        ...
        operacion_inventario: item.operacion_inventario,
  ...
  private _actualizarEntrada() {
    if (this.validarCamposDetalles() === false) {
      ...
      console.log('Datos del formulario antes de actualizar:', this.formularioEntrada.value);
  ...
  private _guardarEntrada() {
    if (this.validarCamposDetalles() === false) {
      ...
      this._calcularTotales();
      console.log('Datos del formulario antes de enviar:', this.formularioEntrada.value);
      this._entradaService
        .guardarFactura({
          ...this.formularioEntrada.value,
          ...{
