...
@Component({
  selector: 'app-recurrente-venta',
  standalone: true,
  imports: [FiltroComponent, SiNoPipe, CommonModule, PaginadorComponent],
  templateUrl: './recurrente-venta.component.html',
  styleUrls: ['./recurrente-venta.component.scss'],
})
export default class RecurrenteVentaComponent implements OnInit, OnDestroy {
    ...
    this._httpService
      ...
      .subscribe({
        next: (respuesta) => {
          ...
          this.router.navigate([`venta/documento/detalle/${respuesta.documentos_creados[0]}`], {
            queryParams: {
              modelo: this._key,
            },
          });
